...
@property (weak, nonatomic) IBOutlet NSLayoutConstraint *mainScrollViewTopSpace;
@end
...
- (void)setupRoundViews {
    ...
    if (self.isFromStats) {
        self.mainSuperView.layer.cornerRadius = 0;
    } else {
...
- (void)updateMainConstraints {
    ...
    //Real-time update
    if ([defaults_object(@"onDemandTracking") boolValue] && self.isFromStats == NO) {
        ...
        if (self.isFromStats) {
            self.mainScrollViewTopSpace.constant = 0;
        } else {
            self.mainScrollViewTopSpace.constant = 74;
        }
